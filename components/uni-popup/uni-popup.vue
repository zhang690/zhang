<template>
	<view v-if="showPopup" class="uni-popup">
		<view :class="[ani, animation ? 'ani' : '', !custom ? 'uni-custom' : '']" :style="maskStyle" class="uni-popup__mask" @click="close(true)" />
		<view :class="[type, ani, animation ? 'ani' : '', !custom ? 'uni-custom' : '']" class="uni-popup__wrapper" @click="close(true)">
			<view class="uni-popup__wrapper-box" @click.stop="clear" :style="getPopupStyle"><slot /></view>
		</view>
	</view>
</template>
...
export default {
	...
	watch: {
		...
		// 		console.log(newName)
		// 		this.showJson(newName);
		// 		// return this.showJson(this.popupStyle);
		//     },
		//     immediate: true
		//     ,deep: true
		//   }
