<template>
	<view class="swiperContent">
		<swiper
			...
			:indicator-dots="indicatorDots && swiperList.length > 1"
		...
		>
			...
			>
				<view v-if="item[imageKey] && !item[videoKey]">
					<image :src="iconURL + item[imageKey]" :style="{ height: swiperHeight + 'px' }" class="imageKey"></image>
					<text
						v-if="textTip"
						class="swiperText"
						:style="{
			...
			<swiper-item class="swiper-item" v-if="swiperList.length == 0"><text></text></swiper-item>
		</swiper>
	</view>
</template>
...
.screen-swiper image {
	width: 668upx !important;
	height: 328upx !important;
	border-radius: 20upx;
	/* box-shadow:0px 2upx 10upx 0px rgba(132, 158, 150,1); */
}
.screen-swiper video,
.swiper-item video {
	width: 100%;
