<template>
	<view class="hx-navbar">
		...
		>
			<block v-if="backgroundImg"><image class="navbgimg" :src="backgroundImg" mode=""></image></block>
			...
			<view :style="{ color: colorInfo, height: height, 'line-height': height }" class="hd hx-navbar__header hx-navbar__content_view">
				<view class="hx-navbar__header-btns hx-navbar__content_view" @tap="onClickLeft" v-if="leftSlot" :style="{ color: colorInfo }">
					<block v-if="leftText.length || leftIcon.length || back">
						...
						<view v-if="leftText.length" :class="{ 'hx-navbar-btn-icon-left': !leftIcon.length }" class="hx-navbar-btn-text hx-navbar__content_view">
							{{ leftText }}
						</view>
					</block>
					<block v-else>
				...
				<view :class="title.length ? 'hx-navbar__header-btns-right' : ''" class="hx-navbar__header-btns hx-navbar__content_view" @tap="onClickRight" v-if="rightSlot">
					<!-- 优先显示图标 -->
					<block v-if="rightIcon.length || rightText.length">
						<view class="hx-navbar__content_view" v-if="rightIcon.length"><uni-icons :type="rightIcon" :color="colorInfo" size="28" /></view>
...
export default {
	...
	created() {
		...
		if (that.fixed && (that.leftSlidiSwitch || that.centerSlidiSwitch || that.rightSlidiSwitch)) {
			that.$nextTick(that.doScroll(that.pageScroll));
		}
	},
	watch: {
		pageScroll(val, oldVal) {
			...
			//滑动切换
			if (that.fixed && (that.leftSlidiSwitch || that.centerSlidiSwitch || that.rightSlidiSwitch)) {
				that.$nextTick(that.doScroll(val));
			}
		},
		//监控透明度变化
