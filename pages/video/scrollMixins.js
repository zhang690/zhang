				...
				let gesTokenObj = BindingX.bind({
					...
					props: [{
						...
						},
						{
							element: leftRef,
							property: 'transform.translateX',
							expression: `${move}!=0&& ${move*this.distanceX}<=0?x+${this.distanceX} : ${this.distanceX}`
						},
						{
							element: rightRef,
							property: 'transform.translateX',
			...
			let result = BindingX.bind({
				...
				if (e.state === 'end' || e.state === 'exit') {
					...
					if (Math.abs(Y) > this.backDistance || quickMove) {
						...
						// this.videoList[this.index].time = 0 //让time始终等于0
						// console.log(this.videoList[this.index],'切换视频')
						uni.$emit('cutVideo')
						//加载视频
